...
- Tudo foi desenhado com os tokens reais de `src/theme` e as medidas de
  ...
  Barlow no carimbo.
- As pranchetas de logotipo marcadas como **proposta** (logo em pé com
  título/subtítulo; logo largo na largura do bloco) ainda **não** existem no
  código — hoje `stamp-layout.ts` amarra a altura do logo ao texto e contém
  proporções acima de 2,4. Os brasões e logos são desenhos genéricos de
  exemplo, não marcas reais.
