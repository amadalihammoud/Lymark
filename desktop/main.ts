...
riteFileSync(filePath, buffer);
  ...
  ipcMain.handle('save-to-gallery', async (event, { bytes, filename }: { bytes: number[]; filename: string }) => {
    try {
      ...
      if (bytes.length > MAX_FILE_SIZE) {
        return { 
status: 'failed', error: 'Arquivo muito grande (máx. 50MB).' };
      }
  ...
  // Handle
r para obter pasta de saída atual
  ...
  // i18n handlers
  const { supportedLocales, defaultLocale, getTranslation } = require('./i18n');
  ...
  ipcMain.handle('translate', async (event, { key }) => {
    return { translation: getTranslation(currentLocale, key) };
  });
 ...
 * de ser lidos de dentro do pacote,
 que é somente leitura. Um script injetado
